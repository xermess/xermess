<script lang="ts">
	...
	import { labelFor, markFor, specFor } from './steps';
...
<DataTable {columns} rows={flows} {empty} {onOpen} label={(flow) => `Edit ${flow.name}`}>
	{#snippet row(flow)}
		<td>
			<span class="names">
				...
				{#each flow.steps as step (step)}
					...
					<Tooltip
						label={spec?.implemented === false
							? `${spec.description} Not run yet.`
							: (spec?.description ?? step)}
					>
